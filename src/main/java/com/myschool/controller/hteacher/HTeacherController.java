...
public class HTeacherController extends BaseController {
	...
	public String showLearningOutcomes(Model model,
		...
		//Long teacherId = getCurrentUser(model).getId();
		...
		//hiển thị bảng điểm
		if(homeroomTeacherService.getStudentByTeacherAndSemester(teacherId, semesters.get(0).getId(), schoolYears.get(0).getId())!=null 
				&& !homeroomTeacherService.getStudentByTeacherAndSemester(teacherId, semesters.get(0).getId(), schoolYears.get(0).getId()).isEmpty()) {
		...
		} else if(homeroomTeacherService.getStudentByTeacherAndSemester(teacherId, semesterId, yearId)!=null 
				&& !homeroomTeacherService.getStudentByTeacherAndSemester(teacherId, semesterId, yearId).isEmpty()) {
			if (semesterId==null && subjectId==null && yearId==null)
				scoreEntities = scoreService.getScoreBySemesterAndSubject(semesters.get(0).getId(), subjects.get(0).getId());
			else scoreEntities = scoreService.getScoreBySemesterAndSubject(semesterId, subjectId); 
		}
		...
		return "giaoviencn/learning-outcomes";
	}
}
